...
var hook = (xhr) => {
  ...
  container.querySelectorAll('.tweet-text[lang="ja"]')
    .forEach((ele) => {
      ...
      const rubify = [...ele.innerText].map(c=>{if(/[\u4E00-\u9FFF]/.test(c)){return `${c}<rt>あ</rt>`} return c;}).join('')
...
//
  // Object.defineProperty(this, 'responseText', { value: JSON.stringify(rawJSON) });

  console.error('replaced.', xhr.responseText);
  // console.error('url:', xhr);
}
